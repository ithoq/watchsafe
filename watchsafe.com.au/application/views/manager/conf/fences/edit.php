...
<script type="text/javascript">
      ...
      function initialize(){
     var mapOptions = {
        zoom: <?=($fence->size==0) ? 5: $fence->size?>,
        center: new google.maps.LatLng(<?=$fence->latitude?>,<?=$fence->longitude?>),
...
<table class='table table-bordered' style="font-size:15px;">
      ...
      <input type="text" name="name" id="name" value="<?=$fence->name?>" placeholder="Geo fence name" />
      <select name='type' id='type'>
               <option value='entry&exit' <?php if ($fence->type=='entry&exit') echo "selected"?> >entry&exit</option>
               <option value='overspeed' <?php if ($fence->type=='overspeed') echo "selected"?> >overspeed</option>
            </select>
            Usage type: <br><br>
            <div id="overspeed" style="display:none;">
              <input type="text" name="overspeed" value="<?=$fence->overspeed?>" placeholder="Overspeed limit" />
              <p style="color:red;"> You must set overspeed limit in KM/h
            </div>
            <div id="time">
              
              <div id="datetimepicker1" class="input-append date">
              <input type="text" name='start' value="<?=$fence->time_start?>"  placeholder='Date from'></input>
              <span class="add-on">
             ...
             <div id="datetimepicker2" class="input-append date">
              <input type="text" name='end' value="<?=$fence->time_end?>"  placeholder='Date to'></input>
              <span class="add-on">
                <i data-time-icon="icon-time" data-date-icon="icon-calendar"></i>
              </span>
            </div>
      ...
      <input type="hidden" name="radius" value="<?=$fence->radius?>" id="radius" />
      <input type="hidden" name="lat" value="<?=$fence->latitude?>" id="lat" />
      <input type="hidden" name="lon" value="<?=$fence->longitude?>" id="lon" />
      <input type="hidden" name="size" value="<?=$fence->longitude?>" id="size" />
            <div align='center'>
            <a href="/manager/fences?imei=<?=$imei?>" class=" btn btn-info btn-large"  >BACK</a>
            <a href="#" class=" btn btn-info btn-large" id="save" >EDIT</a>
      <!--<input type='submit' name='add' value='SAVE' class="btn btn-success btn-large"/>-->
          ...
          <td style="width:75%">
            <input type="text" name="address" id="adress" style="width:400px; height:40px;" />
